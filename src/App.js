...
import { getCurrentUser, signOut, adddata, modifydata, loadfromCloud } from './leanCloud'
...
class App extends Component {
    ...
    render() {
        ...
        return ( < div className = "App" >
            ...
            的手账 {
                this.state.user.id ? < button onClick = { this.signOut.bind(this) } > 登出 < /button> : null}</h
                1 >
                    < div className = "inputWrapper" >
                    < TodoInput content = { this.state.newTodo }
                onChange = { this.changeTitle.bind(this) }
                onSubmit = { this.addTodo.bind(this) }
                /> </div > < ol className = "todoList" > { todos } < /ol> {
        ...
        componentDidUpdate() {
            if (!this.state.todoList.id) {
                let setOwner = (id) => { this.state.todoList.id = id }
